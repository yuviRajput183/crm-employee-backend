...
class AdvisorService {
  ...
  async getAdvisorsForDropdown(req, res, next) {
    ...
    if (user.isOwner) {
      ...
      advisors = await Advisor.find({ groupId: user._id, isActive: true })
        .select("name _id advisorCode")
        .sort({ advisorCode: -1 });
    } else {
      ...
      if (isAdmin) {
        ...
        advisors = await Advisor.find({ createdBy: user._id, isActive: true })
          .select("name _id advisorCode")
          .sort({ advisorCode: -1 });
      } else {
        // Normal Employee: Get all the advisors whose reporting officer is same as reporting officer of the logged in employee.

        advisors = await Advisor.find({ reportingOfficer: user.reportingOfficer, isActive: true })
          .select("name _id advisorCode")
          .sort({ advisorCode: -1 });
      }
    }
